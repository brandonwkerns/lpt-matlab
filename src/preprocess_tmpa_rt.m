...
for dn = DN1-ACCUMULATION_PERIOD/24.0:datenum(0,0,0,DT,0,0):DN2
  ...
  this_raw_file = [this_dir,'/3B42RT.',YYYY,MM,DD,HH,'.7.bin'];
  ...
  if (exist(this_raw_file))
    F = read3b42rt_struct(this_raw_file);

    %% Longitude 0 - 360
    %F.lon = [F.lon(721:1440), 360.0 + F.lon(1:720)];
    %F.precip = [F.precip(:,721:1440), F.precip(:,1:720)];

    %% RT is 59.875 S/N. To be consistent with research product, limit to 49.875 S/N
    F.lat = F.lat(41:end-40);
    F.precip = F.precip(41:end-40, :);
