...
dir = '../data/trmm_keep_overlapping_tracks/processed/g20_72h/thresh12/timeclusters'
out_dir = '../plots/trmm_keep_overlapping_tracks/processed/g20_72h/thresh12/systems'
plt.close('all')
...
clumps_file = '/home/orca/bkerns/data/lpt/trmm/processed/g20_72h/thresh12/identify_eastward_propagation/clumps_of_worms.txt'
...
D_all = np.vstack((D_mc_crossing, D_non_mc_crossing, D_wpac))
mjo={}
mjo['year'] = D_all[:,0]
mjo['idx'] = D_all[:,1]
...
for fn in sorted(glob.glob(dir+'/TIMECLUSTERS_*.lptALL.nc')):

    print(fn, flush=True)
    year = int(fn[-31:-27])
    
    out_str = fn[-31:-10]
    fout = (out_dir + '/lpt_mjo_system_tracks_' + out_str + '__map.png')
    ...
    for cnum in sorted(np.unique(clumps_this_year['clump_num'])):
        ...
        for idx in np.unique(clumps_this_year['idx'][clumps_this_year['clump_num'] == cnum]):
            ...
            if np.logical_and(mjo['year'] == year, mjo['idx'] == idx).any():
                ...
                map.plot(x[0], y[0], 'ro', markersize=5, zorder=1000)
                map.plot(x[-1], y[-1], 'mx', markersize=7, zorder=1000, markeredgewidth=2.0)
    ...
    xt, yt = map(320.0, 45.0)
    plt.text(xt, yt, "N = " + str(mjo_count), fontsize=14, fontweight='bold')
